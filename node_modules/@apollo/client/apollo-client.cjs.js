...
function getFragmentQueryDocument(document, fragmentName) {
    ...
    document.definitions.forEach(function (definition) {
        if (definition.kind === 'OperationDefinition') {
            throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(45) : new invariant.InvariantError("Found a " + definition.operation + " operation" + (definition.name ? " named '" + definition.name.value + "'" : '') + ". " +
    ...
    if (typeof actualFragmentName === 'undefined') {
        process.env.NODE_ENV === "production" ? invariant.invariant(fragments.length === 1, 46) : invariant.invariant(fragments.length === 1, "Found " + fragments.length + " fragments. `fragmentName` must be provided when there is not exactly 1 fragment.");
...
function getFragmentFromSelection(selection, fragmentMap) {
    switch (selection.kind) {
        ...
        case 'FragmentSpread': {
            ...
            process.env.NODE_ENV === "production" ? invariant.invariant(fragment, 47) : invariant.invariant(fragment, "No fragment named " + selection.name.value + ".");
            return fragment;
        }
        default:
            return null;
...
function valueToObjectRepresentation(argObj, name, value, variables) {
    if (isIntValue(value) || isFloatValue(value)) {
        ...
        throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(26) : new invariant.InvariantError("The inline argument \"" + name.value + "\" of kind \"" + value.kind + "\"" +
...
function validateOperation(operation) {
    ...
    for (var _i = 0, _a = Object.keys(operation); _i < _a.length; _i++) {
        ...
        if (OPERATION_FIELDS.indexOf(key) < 0) {
            throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(25) : new invariant.InvariantError("illegal argument: " + key);
        }
    }
    return operation;
}
...
function shouldInclude(selection, variables) {
    ...
    return getInclusionDirectives(selection.directives).every(function (_a) {
        ...
        if (ifArgument.value.kind === 'Variable') {
            ...
            process.env.NODE_ENV === "production" ? invariant.invariant(evaledValue !== void 0, 41) : invariant.invariant(evaledValue !== void 0, "Invalid variable referenced in @" + directive.name.value + " directive.");
        }
        else {
...
function getInclusionDirectives(directives) {
    return directives ? directives.filter(isInclusionDirective).map(function (directive) {
        ...
        process.env.NODE_ENV === "production" ? invariant.invariant(directiveArguments && directiveArguments.length === 1, 42) : invariant.invariant(directiveArguments && directiveArguments.length === 1, "Incorrect number of arguments for the @" + directiveName + " directive.");
        var ifArgument = directiveArguments[0];
        process.env.NODE_ENV === "production" ? invariant.invariant(ifArgument.name && ifArgument.name.value === 'if', 43) : invariant.invariant(ifArgument.name && ifArgument.name.value === 'if', "Invalid argument for the @" + directiveName + " directive.");
        var ifValue = ifArgument.value;
        process.env.NODE_ENV === "production" ? invariant.invariant(ifValue &&
            (ifValue.kind === 'Variable' || ifValue.kind === 'BooleanValue'), 44) : invariant.invariant(ifValue &&
    ...
    LocalState.prototype.prepareContext = function (context) {
        ...
        var newContext = tslib.__assign(tslib.__assign({}, context), { cache: cache, getCacheKey: function (obj) {
                if (cache.config) {
                    ...
                    process.env.NODE_ENV === "production" ? invariant.invariant(false, 23) : invariant.invariant(false, 'To use context.getCacheKey, you need to use a cache that has ' +
...
var QueryStore = (function () {
    ...
    QueryStore.prototype.initQuery = function (query) {
        ...
        process.env.NODE_ENV === "production" ? invariant.invariant(!previousQuery ||
            ...
            equality.equal(previousQuery.document, query.document), 50) : invariant.invariant(!previousQuery ||
            previousQuery.document === query.document ||
...
var QueryManager = (function () {
    ...
    QueryManager.prototype.stop = function () {
        ...
        this.fetchQueryRejectFns.forEach(function (reject) {
            reject(process.env.NODE_ENV === "production" ? new invariant.InvariantError(27) : new invariant.InvariantError('QueryManager stopped while query was in flight'));
        });
    };
    ...
    QueryManager.prototype.watchQuery = function (options, shouldSubscribe) {
        ...
        process.env.NODE_ENV === "production" ? invariant.invariant(options.fetchPolicy !== 'standby', 30) : invariant.invariant(options.fetchPolicy !== 'standby', 'client.watchQuery cannot be called with fetchPolicy set to "standby"');
    ...
    QueryManager.prototype.query = function (options) {
        var _this = this;
        process.env.NODE_ENV === "production" ? invariant.invariant(options.query, 31) : invariant.invariant(options.query, 'query option is required. You must specify your GraphQL document ' +
            'in the query option.');
        process.env.NODE_ENV === "production" ? invariant.invariant(options.query.kind === 'Document', 32) : invariant.invariant(options.query.kind === 'Document', 'You must wrap the query string in a "gql" tag.');
        process.env.NODE_ENV === "production" ? invariant.invariant(!options.returnPartialData, 33) : invariant.invariant(!options.returnPartialData, 'returnPartialData option only supported on watchQuery.');
        process.env.NODE_ENV === "production" ? invariant.invariant(!options.pollInterval, 34) : invariant.invariant(!options.pollInterval, 'pollInterval option only supported on watchQuery.');
        return new Promise(function (resolve, reject) {
    ...
    QueryManager.prototype.clearStore = function () {
        this.fetchQueryRejectFns.forEach(function (reject) {
            reject(process.env.NODE_ENV === "production" ? new invariant.InvariantError(35) : new invariant.InvariantError('Store reset while query was in flight (not completed in link chain)'));
        });
        var resetIds = [];
    ...
    QueryManager.prototype.getQueryWithPreviousResult = function (queryIdOrObservable) {
        ...
        if (typeof queryIdOrObservable === 'string') {
            ...
            process.env.NODE_ENV === "production" ? invariant.invariant(foundObservableQuery, 36) : invariant.invariant(foundObservableQuery, "ObservableQuery with this id doesn't exist: " + queryIdOrObservable);
    ...
    QueryManager.prototype.startPollingQuery = function (options, queryId, listener) {
        ...
        process.env.NODE_ENV === "production" ? invariant.invariant(pollInterval, 37) : invariant.invariant(pollInterval, 'Attempted to start a polling query without a polling interval.');
        if (!this.ssrMode) {
...
var OperationData = (function () {
    ...
    OperationData.prototype.refreshClient = function () {
        ...
        process.env.NODE_ENV === "production" ? invariant.invariant(!!client, 48) : invariant.invariant(!!client, 'Could not find "client" in the context or passed in as an option. ' +
    ...
    OperationData.prototype.verifyDocumentType = function (document, type) {
        ...
        process.env.NODE_ENV === "production" ? invariant.invariant(operation.type === type, 49) : invariant.invariant(operation.type === type, "Running a " + requiredOperationName + " requires a graphql " +
